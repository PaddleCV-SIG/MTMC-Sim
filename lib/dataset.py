...
import tarfile
import zipfile
from utils.os_utils import read_image, download_url, mkdir_if_missing
...
class Dataset(IterableDataset):
    ...
    def __add__(self, other):
        ...
        if isinstance(train[0][0], str):
            ...
            print('dataset requested is not available!')
...
class ImageDataset(Dataset):
    ...
    def __iter__(self):
        """
        tell dataloader how to sample idx
        """
        self.start = 0
        self.end = self.__len__()
        self.pick_order = np.array([i for i in range(self.end)])
        if self.mode == "train":
            # In this case we shuffle training set
            np.random.shuffle(self.pick_order)
        for i in range(len(self.pick_order)):
            idx = self.pick_order[i]
